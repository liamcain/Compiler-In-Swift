...
//        let digitWidth = (maxDigits as NSString).sizeWithAttributes(textAttributes()).width * 2 + RULER_MARGIN
        let digitWidth = (maxDigits as NSString).sizeWithAttributes([
            NSFontAttributeName: NSFont.labelFontOfSize(NSFont.systemFontSizeForControlSize(NSControlSize.SmallControlSize)),
            NSForegroundColorAttributeName: NSColor(calibratedWhite: 0.42, alpha: 1.0)
            ]).width * 2 + CGFloat(RULER_MARGIN)
        let defaultThickness = CGFloat(DEFAULT_THICKNESS)
        return defaultThickness// digitWidth > defaultThickness  ? digitWidth : defaultThickness
    }
...
//            line.stroke()
            ...
            for var lineNumber = startChange; lineNumber <= endChange; lineNumber++ {
                ...
                if lineRectCount > 0 {
                    ...
                    let labelSize = labelText.sizeWithAttributes([
//                        NSFontAttributeName: NSFont.labelFontOfSize(NSFont.systemFontSizeForControlSize(NSControlSize.MiniControlSize)),
                    ...
                    labelText.drawInRect(lineNumberRect, withAttributes: [
//                        NSFontAttributeName: NSFont.labelFontOfSize(NSFont.systemFontSizeForControlSize(NSControlSize.MiniControlSize)),
                        NSFontAttributeName: NSFont.labelFontOfSize(NSFont.systemFontSizeForControlSize(NSControlSize.SmallControlSize)),
                        NSForegroundColorAttributeName: NSColor(calibratedWhite: 0.42, alpha: 1.0)
                        ])
